...
function getData(){
    axios.get('https://api.vschool.io/dalexander/todo')
...
function listData(arr){
    ...
    for(let i = 0; i < arr.length; i++){
        ...
        delBtn.addEventListener('click', ()=>{
            axios.delete("https://api.vschool.io/dalexander/todo/" + arr[i]._id)
                .then(getData)
        })
        ...
        doneBtn.addEventListener('click', () => {
                if (arr[i].completed === false){
                    axios.put("https://api.vschool.io/dalexander/todo/" + arr[i]._id, updates)
                        .then(getData)
                        .catch(err => console.log(err))    
                } else if (arr[i].completed === true){
                    axios.put("https://api.vschool.io/dalexander/todo/" + arr[i]._id, otherUpdates)
                        .then(getData)
                        .catch(err => console.log(err))
                }
        })
...
todoForm.addEventListener("submit", function(e){
    ...
    axios.post("https://api.vschool.io/dalexander/todo/", newTodo)
        .then(getData)
